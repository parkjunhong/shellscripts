...
FILE_DIR="$OUTPUT_ROOT_DIR/$(basename $INPUT_FILE)-ex"
if [ -d $FILE_DIR ];then
	rm -rf $FILE_DIR
fi
# 디렉토리 생성
...
write-line(){
	...
	do
		c="${string:$i:1}"
		if [ "$c" == "$sep" ];then
			if [ $clmnNum -eq $CLMN_POS ];then
				write-column $ln "$buf" $clmnNum
				brk=1
				break
			fi
			((clmnNum++))
		...
		printf "\r\033[K + + + %-12s: %'d | %s" "Column.Reading" $_total_data_count ${_progress[((${_total_data_count}%${PROG_MOD}))]}
		((_total_data_count++))
	done

	# 마지막 데이터
	if [ $brk -eq 0 ] && [ $clmnNum -eq $CLMN_POS ];then
		write-column $ln "$buf" $clmnNum
	fi
}
