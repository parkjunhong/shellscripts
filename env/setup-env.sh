...
help(){
  ...
  echo "  --bin           ~/bin 디렉토리 생성 및 PATH 환경변수에 추가합니다."
  echo "  --vim           vim 편집기를 설치합니다."
  echo "  --prompt           .bashrc 파일에 git branch 표시 프롬프트를 설정합니다."
  ...
  echo "  --sudoers       ymtech 사용자를 sudoers에 등록하고 특정 명령어 비번 생략을 설정합니다."
}
...
_install_vim_cli() {
  _install_curl
  echo "[진행] vim-cli 무조건 설치 중..."
  local bin_dir="$HOME/bin"
  local comp_dir="/etc/bash_completion.d"
  
  local temp_bin="/tmp/vim-cli"
  ...
  if [ -f "$bin_dir/vim-cli" ]; then
    if ! cmp -s "$temp_bin" "$bin_dir/vim-cli"; then
      read -p "> vim-cli 내용이 다릅니다. 업데이트하시겠습니까? (y/n): " ch
      if [[ "$ch" == "y" || "$ch" == "Y" ]]; then
        cp "$temp_bin" "$bin_dir/vim-cli" && chmod +x "$bin_dir/vim-cli"
    ...
    mkdir -p "$bin_dir" && cp "$temp_bin" "$bin_dir/vim-cli" && chmod +x "$bin_dir/vim-cli"
    echo " - vim-cli 실행 파일을 설치했습니다."
  fi

  local temp_comp="/tmp/vim-cli.completion"
  curl -sLo "$temp_comp" "https://raw.githubusercontent.com/parkjunhong/shellscripts/refs/heads/main/sys/vim-cli.completion" || error_exit "vim-cli completion 다운로드 실패" "$LINENO"
  
  if [ -f "$comp_dir/vim-cli.completion" ]; then
    if ! cmp -s "$temp_comp" "$comp_dir/vim-cli.completion"; then
      read -p "> vim-cli.completion 내용이 다릅니다. 업데이트하시겠습니까? (y/n): " ch
      if [[ "$ch" == "y" || "$ch" == "Y" ]]; then
        sudo cp "$temp_comp" "$comp_dir/vim-cli.completion"
        echo " - vim-cli completion 파일이 업데이트되었습니다."
      else
        echo " - vim-cli completion 파일 설치를 유지합니다."
      fi
    else
      echo " - vim-cli completion 파일이 이미 최신입니다."
    fi
  else
    sudo cp "$temp_comp" "$comp_dir/vim-cli.completion"
    echo " - vim-cli completion 파일을 설치했습니다."
  fi
  
  # Bash Completion 현재 쉘 즉시 적용
  if [ -f "$comp_dir/vim-cli.completion" ]; then
    source "$comp_dir/vim-cli.completion" 2>/dev/null || true
    echo " - [System] vim-cli 자동완성이 현재 터미널에 즉시 적용되었습니다."
  fi
}
...
setup_ssh_key() {
  ...
  if ! grep -q "ymtech for internal" "$auth_file"; then
...
setup_sudoers() {
  ...
  if [ "$PKG_MANAGER" == "apt" ]; then
    echo "$target_user ALL=(ALL) NOPASSWD: /usr/bin/apt, /usr/bin/apt-get, /usr/bin/cat, /usr/bin/systemctl, /usr/bin/vi, /usr/bin/vim, /bin/ps, /bin/netstat" | sudo tee -a "$sudoers_file" > /dev/null
  elif [ "$PKG_MANAGER" == "dnf" ]; then
    echo "$target_user ALL=(ALL) NOPASSWD: /usr/bin/dnf, /usr/bin/cat, /usr/bin/systemctl, /usr/bin/vi, /usr/bin/vim, /bin/ps, /bin/netstat" | sudo tee -a "$sudoers_file" > /dev/null
  fi
